...
class Ride extends Model
{
	public static function create($car_id, $description, $nb_free_seats, $value_luggage, $date, $creation_date, $user_id = null) {
		...
		return self::getDB()->request(
			"INSERT INTO ".static::getTableName()." VALUES (4,:user_id, :car_id, :description, :nb_free_seats, :value_luggage, :date, :creation_date);",
			[
				'user_id' => $user_id,
				'car_id' => $car_id,
	...
	public static function getDifferentFromUser($user_id = null) {
		if ($user_id === null)
			$user_id = $_SESSION['id'];

		return static::getDB()->request(
			'SELECT * FROM '.static::getTableName().' r, passagers p WHERE r.user_id != :id and r.id NOT IN
        (
        SELECT  p.ride_id
        FROM    passagers p WHERE p.user_id = :id2) ',
			[
				'id' => $user_id,
				'id2' => $user_id
			]
